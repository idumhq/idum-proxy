...
from idum_proxy.networking.connection_pooling.connectors.context_connector import get_context_connector
from idum_proxy.networking.connection_pooling.connectors.event_loop_connector_manager import event_loop_manager
from idum_proxy.networking.connection_pooling.connectors.thread_local_connector import thread_local_connector
from idum_proxy.networking.connection_pooling.tracing.default_trace_handler import DefaultTraceHandlers, TraceHandlers
...
class HTTPClient:
    ...
    def __init__(self, connector_strategy: str = "context_var",
    ...
    def _create_trace_config(self) -> TraceConfig | None:
        ...
        trace_config.on_connection_create_start.append(handlers.on_connection_create_start)
    ...
    async def _setup_resources(self):
        ...
        if self.tcp_connector is None:
            if self.connector_strategy == "dedicated":
                self.tcp_connector = TCPConnector(ssl=True, keepalive_timeout=75, limit=10)
                ...
                raise ValueError(f"Invalid connector_strategy: {self.connector_strategy}")
